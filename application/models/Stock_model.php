...
class Stock_model extends CI_Model {
	...
	function invoice_submit($invNO,$stockid,$supplier_id,$mainDate,$mainDeliDate,$net_total,$pro_code,$pro_name,$pro_id,$units,$qty,$price,$net,$minvoice)
	{
		...
		for($i=0;$i<$proInv;$i++)
		{
			if(($qty[$i]!='' && $qty[$i]>0))
			{
				...
				$query=$this->db->query("INSERT INTO purchase_invoice_details VALUES('','$inv_id','$minvoice','$stockid','$supplier_id','$pro_name[$i]','$pro_code[$i]','$pro_id[$i]','','','$units[$i]','$qty[$i]','','','','$price[$i]','$net[$i]','','','$mainDate','','','','')");
